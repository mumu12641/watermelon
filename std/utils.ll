...
entry: 
  ...
  br i1 %str1_null, label %handle_str1_null, label %check_str2 handle_str1_null: 
  ; preds = %entry
  br i1 %str2_null, label %both_null, label %str1_null_str2_valid check_str2: 
  ; preds = %entry
  br i1 %str2_null, label %str2_null_str1_valid, label %both_valid both_null: 
  ; preds = %handle_str1_null
  %empty_str = call noalias i8* @gc_alloc(i64 1) 
  store i8 0, i8* %empty_str, align 1 
  ret i8* %empty_str str1_null_str2_valid: 
  ; preds = %handle_str1_null
  ...
  ret i8* %result2_only str2_null_str1_valid: 
  ; preds = %check_str2
  ...
  ret i8* %result1_only both_valid: 
  ; preds = %check_str2
  ...
  br i1 %malloc_success, label %copy_strings, label %malloc_failed malloc_failed: 
  ; preds = %both_valid
  ret i8* null copy_strings: 
  ; preds = %both_valid
...
entry: 
  ...
  br i1 %alloc_success, label %convert, label %alloc_failed alloc_failed: 
  ; preds = %entry
  ret i8* null convert: 
  ; preds = %entry
  ...
  br i1 %convert_success, label %success, label %convert_failed convert_failed: 
  ; preds = %convert
  ret i8* null success: 
  ; preds = %convert
  ret i8* %buffer 
...
entry: 
  br i1 %value, label %true_branch, label %false_branch true_branch: 
  ; preds = %entry
  %true_buffer = call noalias i8* @gc_alloc(i64 5) 
  %true_alloc_success = icmp ne i8* %true_buffer, null 
  br i1 %true_alloc_success, label %copy_true, label %true_alloc_failed true_alloc_failed: 
  ; preds = %true_branch
  ret i8* null copy_true: 
  ; preds = %true_branch
  ...
  ret i8* %true_buffer false_branch: 
  ; preds = %entry
  %false_buffer = call noalias i8* @gc_alloc(i64 6) 
  %false_alloc_success = icmp ne i8* %false_buffer, null 
  br i1 %false_alloc_success, label %copy_false, label %false_alloc_failed false_alloc_failed: 
  ; preds = %false_branch
  ret i8* null copy_false: 
  ; preds = %false_branch
...
entry: 
  ...
  br i1 %str_null, label %invalid_input, label %check_empty check_empty: 
  ; preds = %entry
  %len = call i64 @strlen(i8* %str) 
  %is_empty = icmp eq i64 %len, 0 
  br i1 %is_empty, label %invalid_input, label %convert convert: 
  ; preds = %check_empty
  %result = call i32 @atoi(i8* %str) 
  ret i32 %result invalid_input: 
  ; preds = %check_empty, %entry
  ret i32 0 
}
...
entry: 
  ...
  ret float %result_float invalid_input: 
  ; preds = %check_empty, %entry
...
entry: 
  %str_null = icmp eq i8* %str, null 
  br i1 %str_null, label %return_false, label %check_empty check_empty: 
  ; preds = %entry
  %len = call i64 @strlen(i8* %str) 
  %is_empty = icmp eq i64 %len, 0 
  br i1 %is_empty, label %return_false, label %check_true check_true: 
  ; preds = %check_empty
  %true_str_ptr = getelementptr inbounds [5 x i8], [5 x i8]* @bool_true_str, i64 0, i64 0 
  %cmp_true = call i32 @strcmp(i8* %str, i8* %true_str_ptr) 
  %is_true = icmp eq i32 %cmp_true, 0 
  br i1 %is_true, label %return_true, label %check_false check_false: 
  ; preds = %check_true
  %false_str_ptr = getelementptr inbounds [6 x i8], [6 x i8]* @bool_false_str, i64 0, i64 0 
  %cmp_false = call i32 @strcmp(i8* %str, i8* %false_str_ptr) 
  %is_false = icmp eq i32 %cmp_false, 0 
  br i1 %is_false, label %return_false, label %return_false return_true: 
  ; preds = %check_true
  ret i1 true return_false: 
  ; preds = %check_false, %check_false, %check_empty, %entry
  ret i1 false 
}

...
void IRScope::add(const std::string& key, llvm::Value* value)
{
    map[key] = value;
}
const llvm::Value* IRScope::find(const std::string& key)
{
...
{
    scopes.emplace_back(IRScope(name));
}
...
void IRValueTable::add(const std::string& key, llvm::Value* value)
{
    if (!scopes.empty()) {
        scopes.back().add(key, value);
    }
}
const llvm::Value* IRValueTable::find(const std::string& key)
{
    for (auto it = scopes.rbegin(); it != scopes.rend(); ++it) {
        const llvm::Value* value = it->find(key);
        if (value) {
            return value;
        }
    }
